...
char DEST_PHONE[21]; // Numéro de téléphone modifiable
...
float GUST_FACTOR = 1.5; // Rafale = 1.5x la vitesse moyenne
float MIN_GUST_DIFF = 5.0; // Différence minimale absolue en km/h
unsigned long GUST_MIN_DURATION = 120000; // 2 minutes en ms
...
float tempOffset = 0.0; // Offset de température
...
constexpr int TEMP_OFFSET_EEPROM_ADDR = 4 * sizeof(uint16_t);
constexpr int DEST_PHONE_EEPROM_ADDR = 5 * sizeof(uint16_t);
constexpr int GUST_FACTOR_EEPROM_ADDR = 5 * sizeof(uint16_t) + 20;
constexpr int MIN_GUST_DIFF_EEPROM_ADDR = 5 * sizeof(uint16_t) + 20 + sizeof(float);
constexpr int GUST_MIN_DURATION_EEPROM_ADDR = 5 * sizeof(uint16_t) + 20 + 2 * sizeof(float);
...
constexpr float ADC_TO_VOLTAGE = 3.3 / 4095.0; // RP2040 ADC 12-bit
...
void loop() {
    ...
    if (currentMillis - last2sMeasure >= MEASURE_INTERVAL_2S) {
        ...
        if (newTemp != DEVICE_DISCONNECTED_C && newTemp >= -40.0 && newTemp <= 100.0) {
            tempBuffer[tempIndex] = newTemp + tempOffset;
        ...
        if (usbConnected) {
            ...
            Serial.print(filteredSpeed / 10.0, 1);  // Vitesse instantanée filtrée
            Serial.print(" km/h (");
            ...
            Serial.print(avg1Min / 10.0, 1);  // Vitesse moyenne
            Serial.print(" km/h) | ");

            // Affichage direction vent
            float direction = (analogRead(WIND_VANE_PIN) * ADC_TO_VOLTAGE / 3.3) * 360.0;
            direction = (direction * (windVaneCoeff / 10.0)) + (windVaneOffset / 10.0);
        ...
        if (numStart && numEnd && contentStart) {
            *numEnd = '\0';  // Termine le numéro
            ...
            if (strstr(upperContent, "METEO0000")) {
                ...
                message += "Vitesse (inst.): " + String(filteredSpeed / 10.0, 1) + " km/h\n";
                message += "Vitesse (moy.): " + String(windSpeed1Min[windSpeed1MinIndex] / 10.0, 1) + " km/h\n";
...
void initializeEEPROM() {
    ...
    if (!isInitialized) {
        ...
        EEPROM.put(TEMP_OFFSET_EEPROM_ADDR, tempOffset);
        EEPROM.put(DEST_PHONE_EEPROM_ADDR, DEST_PHONE);
        EEPROM.put(GUST_FACTOR_EEPROM_ADDR, GUST_FACTOR);
        EEPROM.put(MIN_GUST_DIFF_EEPROM_ADDR, MIN_GUST_DIFF);
        EEPROM.put(GUST_MIN_DURATION_EEPROM_ADDR, GUST_MIN_DURATION);
        EEPROM.commit();
    }
}
...
void loadValuesFromEEPROM() {
    ...
    EEPROM.get(TEMP_OFFSET_EEPROM_ADDR, tempOffset);
    EEPROM.get(DEST_PHONE_EEPROM_ADDR, DEST_PHONE);
    EEPROM.get(GUST_FACTOR_EEPROM_ADDR, GUST_FACTOR);
    EEPROM.get(MIN_GUST_DIFF_EEPROM_ADDR, MIN_GUST_DIFF);
    EEPROM.get(GUST_MIN_DURATION_EEPROM_ADDR, GUST_MIN_DURATION);
}
...
void measureWindSpeed() {
    ...
    float speedMs = (voltage / 3.3) * 32.5;
    if (speedMs < 0) speedMs = 0;

    float adjustedSpeed = speedMs * (anemometerCoeff / 10.0) + (anemometerOffset / 10.0);
    uint16_t speedKmh = static_cast<uint16_t>(adjustedSpeed * 3.6 * 10);

    speedBuffer[speedBufferIndex] = speedKmh / 10.0;
    speedBufferIndex = (speedBufferIndex + 1) % 30;

    // Détection des rafales
    updateGustDetection(speedKmh / 10.0);
...
void processCommand(String command) {
    ...
    if (command == "?") {
        ...
        Serial.println("TO=<val> - Offset température (ex: TO=0.5)");
        Serial.println("PN=<val> - Numéro de téléphone (ex: PN=+123456789)");
        Serial.println("GF=<val> - Facteur de rafale (ex: GF=1.5)");
        Serial.println("GD=<val> - Différence minimale de rafale (ex: GD=5.0)");
        Serial.println("GT=<val> - Durée minimale de rafale (ex: GT=120000)");
        ...
        Serial.print(anemometerCoeff / 10.0, 1);
        Serial.print(", Offset: ");
        Serial.println(anemometerOffset / 10.0, 1);
        Serial.print("Girouette - Coeff: ");
        Serial.print(windVaneCoeff / 10.0, 1);
        Serial.print(", Offset: ");
        Serial.println(windVaneOffset / 10.0, 1);
        Serial.print("Température - Offset: ");
        Serial.println(tempOffset, 1);
        Serial.print("Numéro de téléphone: ");
        Serial.println(DEST_PHONE);
        Serial.print("Facteur de rafale: ");
        Serial.println(GUST_FACTOR, 1);
        Serial.print("Différence minimale de rafale: ");
        Serial.println(MIN_GUST_DIFF, 1);
        Serial.print("Durée minimale de rafale: ");
        Serial.println(GUST_MIN_DURATION);
    }
    else if (command.startsWith("AC=")) {
        anemometerCoeff = command.substring(3).toFloat() * 10;
        Serial.print("Nouveau coefficient anémomètre: ");
        Serial.println(anemometerCoeff / 10.0, 1);
    }
        ...
        Serial.println(windVaneCoeff / 10.0, 1);
    }
        ...
        tempOffset = command.substring(3).toFloat();
        Serial.print("Nouvel offset température: ");
        Serial.println(tempOffset, 1);
    }
    else if (command.startsWith("PN=")) {
        command.substring(3).toCharArray(DEST_PHONE, 21);
        Serial.print("Nouveau numéro de téléphone: ");
        Serial.println(DEST_PHONE);
    }
    else if (command.startsWith("GF=")) {
        GUST_FACTOR = command.substring(3).toFloat();
        Serial.print("Nouveau facteur de rafale: ");
        Serial.println(GUST_FACTOR, 1);
    }
    else if (command.startsWith("GD=")) {
        MIN_GUST_DIFF = command.substring(3).toFloat();
        Serial.print("Nouvelle différence minimale de rafale: ");
        Serial.println(MIN_GUST_DIFF, 1);
    }
    else if (command.startsWith("GT=")) {
        GUST_MIN_DURATION = command.substring(3).toInt();
        Serial.print("Nouvelle durée minimale de rafale: ");
        Serial.println(GUST_MIN_DURATION);
    }
